...
RSpec.describe Article, type: :model do
  describe '#validations' do
    ...
    it 'has an invalid title' do
      ...
      expect(article.errors[:title]).to include("Title can't be blank")
    end

    it 'has an invalid content' do
      article.content = ''
      expect(article).not_to be_valid
      expect(article.errors[:content]).to include("Content Can't be blank")
    end

    it 'has an invalid slug' do
      article.slug = ''
      expect(article).not_to be_valid
      expect(article.errors[:slug]).to include("Blank Slug")
    end
    ...
    it 'validates the uniqueness of the slug' do
      ...
      expect(article2.errors[:slug]).to include('Slug has already been taken')
    end
  end
